# Introduction customer-profile REST API
...
Launch application database instance using a `docker-compose up`:
...
You will need a database running in your cluster, see [DATABASE.md](DATABASE.md#kubernetes) to create the claim.
...
### Intellij
right click on `config/workload.yaml` and use the 'Tanzu -> Apply Workload'

WAIT until the workload is ready

Use the 'Tanzu Panel' to get the URL or `tanzu apps workload get customer-profile`

#### Delivery Error?
Did you remember to create the customer-database claim?

### Tanzu CLI (!!! DANGEROUS if using Intellij Tanzu Developer Plugin)
